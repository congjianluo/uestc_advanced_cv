...
def get_positive_features(train_path_pos, feature_params):
    files = os.listdir(train_path_pos)
    for file in files:
        image = cv2.imread(os.path.join(train_path_pos, file), cv2.IMREAD_GRAYSCALE)
        hog = Hog_descriptor(image, cell_size=feature_params['hog_cell_size']
                             , bin_size=feature_params['template_size'])
